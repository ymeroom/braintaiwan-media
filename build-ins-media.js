// 將 insomnia-media-articles 的 3 篇 .md 轉成「Media（大眾衛教）」風格 HTML
// 沿用 build-dem-media.js 的 markdown 轉換器與 Media 版型（teal 品牌）
const fs = require('fs');
...
const articles = [
  ...
  { md: 'insomnia-sleeping-pills-safety.md', out: 'insomnia-sleeping-pills-safety.html', nav: '② 安眠藥怎麼用', desc: '安眠藥會上癮嗎？談依賴與反彈失眠、安全使用原則，以及為什麼「開始吃就要想好怎麼停」。' },
  { md: 'insomnia-new-drugs.md',             out: 'insomnia-new-drugs.html',             nav: '③ 新型助眠藥',   desc: '新型助眠藥（食慾素受體拮抗劑）和傳統安眠藥差在哪？用白話解析機轉、依賴風險與適合對象。' },
];
...
function seriesBox(items, activeIdx){
  ...
  return `  <div class="series-box">
    <div class="sb-h">失眠衛教系列 · 睡不好怎麼辦</div>
${links}
  </div>`;
}

function page(a, contentHtml, items, activeIdx){
...
<main>
  <span class="article-tag">失眠 · 衛教</span>
  <h1>${esc(a.title)}</h1>
...
parsed.forEach((a, idx) => {
  const html = page(a, a.body, parsed, idx);
  fs.writeFileSync(path.join(OUT, a.out), html, 'utf8');
  console.log('寫出', a.out, '—', a.title);
});

console.log('完成：', parsed.length, '篇 Media 失眠文章');
