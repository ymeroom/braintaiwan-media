// 將 commercial-bbm-article/media-blood-test.md 轉成「Media（大眾衛教）」風格 HTML
// 單篇獨立文章：沿用 build-dem-media.js 的轉換器與版型，去掉系列導覽，改為「延伸閱讀」框
const fs = require('fs');
...
const articles = [
  {
    md: 'media-blood-test.md',
    out: 'dementia-blood-test.html',
    desc: '「抽一管血就能驗失智症」的廣告越來越多，有的還能自費自己驗。神經科醫師用白話說清楚：它驗的其實不是「失智症」，沒症狀的人去驗為什麼常常虛驚一場，以及真的擔心時該怎麼做。',
  },
];

// 延伸閱讀（取代系列導覽）
const RELATED = [
  { href: '/posts/dementia-early-signs.html', label: '延伸閱讀', text: '失智十大警訊與及早就醫：該看哪一科、做哪些檢查' },
  { href: 'https://md.braintaiwan.com/cbm01.html', label: '給專業人員', text: '臨床版：市售「抽血驗失智症」可信嗎？（完整解讀）' },
];

// ───────── markdown 轉換器（與 build-dem-media.js 相同）─────────
...
.bt-fb{background:#1877f2}.bt-line{background:#06c755}.bt-x{background:#111}.bt-copy{background:#00695c}
...
function relatedBox(items){
  const links = items.map(n=>
    `    <a href="${n.href}"><span class="rb-label">${esc(n.label)}</span>${esc(n.text)}</a>`).join('\n');
  return `  <div class="series-box">
    <div class="sb-h">延伸閱讀</div>
${links}
  </div>`;
}
...
.series-box .rb-label{display:inline-block;font-size:8.5pt;font-weight:800;color:#fff;background:#2e7d32;border-radius:6px;padding:1px 8px;margin-right:8px;vertical-align:middle}
...
<main>
  <span class="article-tag">${esc(ARTICLE_TAG)}</span>
  <h1>${esc(a.title)}</h1>
...
articles.forEach(a => {
  ...
  const html = page({ ...a, ...parsed }, parsed.body);
  fs.writeFileSync(path.join(OUT, a.out), html, 'utf8');
  console.log('寫出', a.out, '—', parsed.title);
});
